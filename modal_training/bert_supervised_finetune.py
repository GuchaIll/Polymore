...
def train_and_save_models(
    ...
    output_dir = f'models/{output_dir_suffix}'
    ...
    for target in TARGET_NAMES:
        ...
        for fold_id in range(fold_count):
            ...
            model_path_save = os.path.join(fold_dir, f'polymer_bert_v2_{target}.pth')
            ...
            if os.path.exists(model_path_save) and os.path.exists(scaler_path) and os.path.exists(oof_preds_path):
                ...
                # We need to load metrics to populate all_results if we want a final summary, 
                # but for simplicity in resumption we might skip re-verification or just load if necessary.
                # Assuming simple continuation here.
                continue
...
def training_main_v1():
    train_and_save_models(
        ...
        pretrained_weights_path=[
            ...
            'models/20250825_194726_poly_6epochs_8thresh_rankup/single_split/polymer_bert_rankup.pth',
            'models/20250912_203816_poly_6epochs_2thresh_v2.1_rankup/single_split/polymer_bert_rankup.pth', # 2
        ][2],
        output_dir_suffix='poly_6epochs_2thresh_finetune_v2.1',
    ...
    train_and_save_tuned_models(
        target_names_to_config_paths = {
            'Tg': 'configs/ModernBERT-base_Tg_431945.json', # ModernBERT-base_Tg_432314.json',
            'FFV': 'configs/ModernBERT-base_FFV_53.json', # ModernBERT-base_FFV_52.json',
            'Tc': 'configs/ModernBERT-base_Tc_225.json', # ModernBERT-base_Tc_245.json',
            'Density': 'configs/ModernBERT-base_Density_228.json', # ModernBERT-base_Density_243.json',
            'Rg': 'configs/ModernBERT-base_Rg_13615.json', # ModernBERT-base_Rg_13506.json',
        },
        input_data_path='data/from_host/train.csv', 
        target_names_to_extra_data_config_paths={
            'Tg': 'configs/Tg_config_dmitry_3.json',
