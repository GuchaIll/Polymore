...
def analyze_molecule_high_compute(smiles: str, progress_callback=None) -> dict:
    ...
    logger.info("Generating conformer ensemble...")
    ...
    if not conf_ids:
        logger.warning(f"ETKDG conformer generation failed for {smiles}. Trying fallback random embedding.")
        # Fallback to single random embedding if ETKDG fails
        AllChem.EmbedMolecule(mol, useRandomCoords=True)
        conf_ids = [0]
    
    logger.info(f"Generated {len(conf_ids)} conformers for analysis")
    ...
    logger.info(f"Starting quantum calculation loop for {total_steps} conformers")
    ...
    for i, cid in enumerate(conf_ids):
        ...
        logger.debug(f"Running GFN2-xTB for conformer {i+1}/{total_steps} (ID: {cid})")
        ...
        try:
            ...
            logger.debug(f"Conformer {cid}: Energy={energy} Ha, Dipole={dipole_mag}")
            ...
            try:
                ...
                gaps.append(gap_ev)
                logger.debug(f"Conformer {cid}: HOMO-LUMO gap={gap_ev} eV")
            except Exception as e:
                logger.warning(f"Error calculating orbital gap for conformer {cid}: {e}")
                # Fallback for open-shell or weird convergence
                gaps.append(0.0)
                
        except Exception as e:
            logger.error(f"xTB calculation failed for conformer {cid}: {e}")
            # Skip this conformer but continue with others if possible
            continue

    # 4. ANALYSIS & AGGREGATION
    if progress_callback: progress_callback(95, "Aggregating physics metrics...")
    logger.info("Aggregating physics metrics from quantum calculation results...")
    ...
    logger.debug(f"Flexibility: energy_std={energy_std:.6f}, score={flexibility_score}")
    ...
    logger.debug(f"Strength: avg_dipole={avg_dipole:.4f}, score={strength_score}")
    ...
    logger.debug(f"Reactivity: avg_gap={avg_gap:.4f} eV")
    ...
    logger.debug(f"Degradability score: {degradability_score}")
    ...
    logger.debug(f"Sustainability/Stability score: {stability_score}")
    ...
    logger.info(f"Tier 2 analysis complete: {result}")
    return result
